...
type ChurnRateDataViewerProps = {
  initialTab: string;
  tabNames: string[];
  initialData: (string | number)[][];
};

export default function ChurnRateDataViewer({
  initialTab,
  tabNames,
  initialData,
}: ChurnRateDataViewerProps) {
  ...
  async function handleTabChange(e: React.ChangeEvent<HTMLSelectElement>) {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/churn?tab=${encodeURIComponent(newTab)}`);
      if (!res.ok) {
